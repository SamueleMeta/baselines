...
#BINARY line search
def line_search(pol, newpol, actor_params, rets, alpha, grad, correct=True, max_search_ite=30):
    ...
    for i in range(max_search_ite):
        rho = rho_init + epsilon * grad * alpha
...
def optimize_offline(pol, newpol, actor_params, rets, grad_tol=1e-4, bound_tol=1e-4, max_offline_ite=100, correct_ess=True):
    improvement = 0.
    rho = pol.eval_params()
