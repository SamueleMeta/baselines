...
def learn(make_env, make_policy, *,
    ...
    while True:
        ...
        total_time_sum = np.sum(seg['total_time']) # Total time used to generate a batch
        policy_time_sum = np.sum(seg['policy_cum_time']) # Total time used to inference policy for the batch
        env_time_sum = np.sum(seg['env_time']) # Total time used to step the environment
        policy_ratio = policy_time_sum / total_time_sum
        env_ratio = env_time_sum / total_time_sum
        ...
        with timed('summaries before'):
            ...
            # Time records
            logger.record_tabular("TotalTime", total_time_sum)
            logger.record_tabular("PolicyTime", policy_time_sum)
            logger.record_tabular("EnvTime", env_time_sum)
            logger.record_tabular("PolicyRatio", policy_ratio)
            logger.record_tabular("EnvRatio", env_ratio)
