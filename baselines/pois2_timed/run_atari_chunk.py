...
def train(env, max_iters, num_episodes, horizon, iw_method, iw_norm, natural, bound, delta, gamma, seed, policy, max_offline_iters, njobs=1, chunk=1):
    ...
    def make_env(seed=0):
        ...
        return _thunk
    parallel_env = VecFrameStack(SubprocChunkVecEnv([make_env(i + seed) for i in range(njobs)], chunk_size=chunk), 4)
