...
def line_search_binary(pol, newpol, actor_params,
    ...
    low = 0.
    high = None
    bound_init = newpol.eval_bound(
        actor_params, rets, pol, rmax, normalize, use_rmax, use_renyi, delta)
    # old_delta_bound = 0.
    rho_opt = rho_init
    i_opt = 0.
    delta_bound_opt = 0.
...
def line_search_parabola(pol, newpol, actor_params,
                         rets, alpha, natgrad,
                         normalize=True,
                         use_rmax=True,
                         use_renyi=True,
...
def optimize_offline(pol, newpol, actor_params,
                     rets, grad_tol=1e-4,
                     bound_tol=1e-4,
                     max_offline_ite=100,
                     normalize=True,
...
def learn(env_maker, pol_maker, sampler,
    ...
    for it in range(max_iters):
        ...
        # Display and save higher-order-policy parameters
        rho = pol.eval_params()
        if verbose > 1:
