...
class Screener(object):
    ...
    def __init__(self, max_screens=10, sleep_time=10):
        self.max_screens = np.inf if max_screens == -1 else max_screens
    ...
    @property
    def n_running_screens(self):
        return sum([screen.exists for screen in self.screens])
    ...
    def run(self, commands, name='s'):
        ...
        i = 0
        while i < n_screens:
            if self.n_running_screens < self.max_screens:
                print('Launching screen %s' % i)
                self.launch_screen(name, i, commands[i])
                i += 1
            else:
                print('Waiting...')
...
parser.add_argument('--max_screens', type=int, default=10, help='Maximum number of screen running.')
...
if args.command == 'launch':
    ...
    scr = Screener(max_screens=args.max_screens)
